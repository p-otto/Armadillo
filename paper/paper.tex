...
Additionally, the user-to-role and and role-to-permission mappings have to be updated in the access contract (\texttt{giveRights} and \texttt{assignRole}, respectively), to allow verification of users during the process execution.
...
\begin{figure}
	\centering
\includegraphics[width=0.8\textwidth]{fig/Armadillo.png}
...
Building upon the BPMN viewer provided in the toolkit, we implemented the handling of user clicks on BPMN \emph{Service Tasks}, which allowed us to use the viewer as a process engine mock.
